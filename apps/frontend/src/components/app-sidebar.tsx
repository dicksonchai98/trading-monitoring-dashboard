...
import { ActivityIcon, BarChart3Icon, CandlestickChartIcon, HistoryIcon, ShieldCheckIcon } from "lucide-react"

function readSidebarUser(token: string | null, t: ReturnType<typeof useT>): { name: string; email: string; avatar: string } {
  if (!token) {
    return {
      name: t("user.visitor"),
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const teams = React.useMemo(
    () => [
      { name: t("app.brand"), logo: <CandlestickChartIcon />, plan: t("team.mvp") },
      { name: "Strategy Lab", logo: <BarChart3Icon />, plan: t("team.internal") },
    ],
    [t],
  )
      ...
      },
      {
        ...
        items: [
          ...
          ...(role === "admin"
            ...
            : []),
          { title: t("nav.amplitudeDistribution"), url: "/historical-amplitude-distribution" },
        ],
      },
    ],
    [role, t],
  ...
  const navUtilities = React.useMemo(
    () =>
      role === "admin"
        ? [{ name: t("nav.adminAudit"), url: "/admin/audit", icon: <ShieldCheckIcon /> }]
        : [],
    [role, t],
  )
  const user = React.useMemo(() => readSidebarUser(token, t), [token, t])
  const utilities = React.useMemo(() => (token ? navUtilities : []), [navUtilities, token])
  ...
  return (
    <Sidebar role="complementary" collapsible="icon" {...props}>
      ...
      <SidebarContent>
        ...
        {utilities.length > 0 ? <NavProjects projects={utilities} pathname={location.pathname} /> : null}
      </SidebarContent>
      <SidebarFooter>
        {token ? <NavUserAuthenticated user={user} /> : <NavUserGuest user={user} />}
      </SidebarFooter>
      <SidebarRail />
    </Sidebar>
  )
