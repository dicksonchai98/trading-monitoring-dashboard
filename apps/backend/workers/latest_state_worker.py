"""Latest state worker process entrypoint."""
...
def build_latest_state_worker_runtime() -> StreamProcessingWorkerRuntime:
    return StreamProcessingWorkerRuntime(runner_factory=state.build_latest_state_runner)
...
async def _run_worker(runtime: StreamProcessingWorkerRuntime) -> None:
    ...
    if worker_task in done:
        await worker_task
        return
    logger.info("latest-state worker stop signal received")
    worker_task.cancel()
    try:
        await worker_task
...
def main() -> None:
    parser = argparse.ArgumentParser(description="run latest-state worker")
    _ = parser.parse_args()
