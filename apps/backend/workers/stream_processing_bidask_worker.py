"""BidAsk stream processing worker process entrypoint."""
...
def build_bidask_stream_processing_worker_runtime() -> StreamProcessingWorkerRuntime:
    return StreamProcessingWorkerRuntime(runner_factory=state.build_bidask_aggregator_runner)
...
async def _run_worker(runtime: StreamProcessingWorkerRuntime) -> None:
    ...
    if worker_task in done:
        await worker_task
        return
    logger.info("bidask stream worker stop signal received")
    worker_task.cancel()
    try:
        await worker_task
...
def main() -> None:
    parser = argparse.ArgumentParser(description="run bidask stream processing worker")
    _ = parser.parse_args()
