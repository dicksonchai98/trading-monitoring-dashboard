...
class MarketSummary1mModel(Base):
    ...
    yesterday_estimated_turnover: Mapped[float | None] = mapped_column(Float, nullable=True)
    estimated_turnover_diff: Mapped[float | None] = mapped_column(Float, nullable=True)
    estimated_turnover_ratio: Mapped[float | None] = mapped_column(Float, nullable=True)
    futures_code: Mapped[str | None] = mapped_column(String(16), nullable=True)
    futures_price: Mapped[float | None] = mapped_column(Float, nullable=True)
    spread: Mapped[float | None] = mapped_column(Float, nullable=True)
    spread_day_high: Mapped[float | None] = mapped_column(Float, nullable=True)
    spread_day_low: Mapped[float | None] = mapped_column(Float, nullable=True)
    spread_strength: Mapped[float | None] = mapped_column(Float, nullable=True)
    spread_status: Mapped[str | None] = mapped_column(String(32), nullable=True)
