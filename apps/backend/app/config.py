...
INDEX_CONTRIBUTION_ENABLED = _env_bool("INDEX_CONTRIBUTION_ENABLED", False)
INDEX_CONTRIBUTION_ENV = os.getenv("INDEX_CONTRIBUTION_ENV", INGESTOR_ENV)
INDEX_CONTRIBUTION_GROUP = os.getenv("INDEX_CONTRIBUTION_GROUP", "index-contrib:spot")
INDEX_CONTRIBUTION_CONSUMER = os.getenv("INDEX_CONTRIBUTION_CONSUMER", "index-contrib-1")
INDEX_CONTRIBUTION_STREAM_KEY = os.getenv(
    "INDEX_CONTRIBUTION_STREAM_KEY", f"{INDEX_CONTRIBUTION_ENV}:stream:spot"
)
INDEX_CONTRIBUTION_READ_COUNT = _env_int("INDEX_CONTRIBUTION_READ_COUNT", 200)
INDEX_CONTRIBUTION_BLOCK_MS = _env_int("INDEX_CONTRIBUTION_BLOCK_MS", 1000)
INDEX_CONTRIBUTION_CLAIM_IDLE_MS = _env_int("INDEX_CONTRIBUTION_CLAIM_IDLE_MS", 30000)
INDEX_CONTRIBUTION_CLAIM_COUNT = _env_int("INDEX_CONTRIBUTION_CLAIM_COUNT", 200)
INDEX_CONTRIBUTION_CODE = os.getenv("INDEX_CONTRIBUTION_CODE", "TSE001")
INDEX_CONTRIBUTION_WEIGHTS_FILE = os.getenv(
    "INDEX_CONTRIBUTION_WEIGHTS_FILE", "infra/config/tse001_weights.json"
)
INDEX_CONTRIBUTION_SECTOR_MAPPING_FILE = os.getenv(
    "INDEX_CONTRIBUTION_SECTOR_MAPPING_FILE", "infra/config/tse001_sector_mapping.json"
)
INDEX_CONTRIBUTION_INDEX_PREV_CLOSE = float(os.getenv("INDEX_CONTRIBUTION_INDEX_PREV_CLOSE", "0"))
INDEX_CONTRIBUTION_REDIS_MAX_RETRIES = _env_int("INDEX_CONTRIBUTION_REDIS_MAX_RETRIES", 3)
INDEX_CONTRIBUTION_REDIS_RETRY_BACKOFF_MS = _env_int(
    "INDEX_CONTRIBUTION_REDIS_RETRY_BACKOFF_MS", 50
)
INDEX_CONTRIBUTION_REDIS_TTL_SECONDS = _env_int(
    "INDEX_CONTRIBUTION_REDIS_TTL_SECONDS", 60 * 60 * 24 * 3
)
INDEX_CONTRIBUTION_DB_MAX_RETRIES = _env_int("INDEX_CONTRIBUTION_DB_MAX_RETRIES", 3)
INDEX_CONTRIBUTION_DB_RETRY_BACKOFF_MS = _env_int("INDEX_CONTRIBUTION_DB_RETRY_BACKOFF_MS", 200)
