...
3. Implement a single-plan subscription flow using Stripe Checkout + Webhook.
4. Deploy locally via Docker Compose with architecture prepared for future cloud migration.

### 2.2 Non-Goals (MVP)

1. Full historical scraping/statistical engine.
2. Multi-plan subscription tiers.
3. Full multi-instrument coverage (spot/options/institutional data).
4. WebSocket migration in MVP.
...
5. Subscription intent -> Stripe checkout -> webhook -> active subscription flow completes successfully.
...
9. Stripe webhook and subscription state transitions.
...
5. `billing_provider`: Stripe checkout session creation, webhook signature validation, and customer portal support.
...
2. Backend creates Stripe Checkout Session and redirects user to Stripe hosted page.
3. Stripe sends webhook event to backend after checkout/payment result.
4. Subscription state transitions based on webhook event.
...
3. System must activate subscription using Stripe webhook callbacks.
...
4. `POST /billing/webhooks/stripe`
5. `POST /billing/portal-session`
6. `GET /realtime/near-month` (SSE)
...
2. Stripe webhook endpoint must validate Stripe signature header.
...
- Subscription intent -> Stripe webhook activation.
...
3. Stripe webhook successfully activates subscription status.
...
4. Risk: Stripe webhook contracts change or are mis-handled in implementation.

- Mitigation: pin API version and maintain event mapping tests for Stripe webhook contracts.
...
3. Expand Stripe billing lifecycle (upgrade/downgrade/cancel at period end).
...
1. Stripe API version pinning and upgrade policy.
